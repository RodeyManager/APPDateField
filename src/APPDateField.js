...
;(function(){
    ...
    var gVars = {
        ...
        tpl: '<div class="app-s-main">'+
                '<div class="app-s-top">'+
                    '<span class="app-s-btnCancel">取消</span>'+
                    '<span class="app-s-title">选择时间</span>'+
                    '<span class="app-s-btnOk">完成</span>'+
                '</div>'+
    ...
    var APPDateField = function(selectElm, options, complete, cancel){
        ...
        this.opts = {
            ...
            upYear: 'number' === typeof options.downYear ? options.downYear : 60,
    ...
    APPDateField.prototype.setCurrentTXT = function(valList){
        ...
        function _getValue(val, sufix, commer){
            return opts.isCN ? (val + sufix) : (self.dateFm(val) + commer);
        }
    ...
    APPDateField.prototype._scrollTop = function(elObj, y){
        ...
        _scrollTo(elObj[0], y, false, this, false);
        //设置凸镜效果
    ...
    APPDateField.prototype._events = function(){
        ...
        if(this.opts.data){
            new APPTouch(app_y_con[0], app_main[0], opts, this);
        }else{
        ...
        //完成和取消按钮
        this.app_btnOk.bind('touchend', holdEvent(this, this._complete));
        this.app_btnCancel.bind('touchend', holdEvent(this, this._cancel));

        this.app_btnOk.bind('touchstart', this.endEvent);
        this.app_btnCancel.bind('touchstart', this.endEvent);
    ...
    APPDateField.prototype._complete = function(evt){
        ...
        if(this.opts.data){
            //自定义数据存在
            var li = this.getCurrChangeElm(this.app_y_con);
            vlist = li.text();
            this.changeElement = li[0];
        }else{
            //日期时间选择
            vlist = [];
    ...
    APPTouch.prototype = {
        ...
        _scrollTop: function(el, y, flag){
            var isGetMonth2Days = this.callTarget.opts.showTime ? false : true;
            _scrollTo(el, y, flag, this, isGetMonth2Days);
        }
